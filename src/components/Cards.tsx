...
export function ServiceCard({ service, locale }: { service: Service; locale: Locale }) {
  ...
  return (
    <Link href={localizedPath(locale, `xidmetler/${service.slug}`)} className="group glass block h-full rounded-2xl p-6 transition duration-200 motion-safe:hover:-translate-y-1">
      ...
      <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-violet-200">
...
export function PackageCard({ pack, locale }: { pack: Package; locale: Locale }) {
  ...
  return (
    <div className={`glass group relative flex h-full flex-col rounded-2xl p-6 transition duration-200 motion-safe:hover:-translate-y-1 ${pack.featured ? "shadow-[0_20px_70px_rgba(139,92,246,0.12)]" : ""}`}>
      {pack.featured ? <span className="absolute right-4 top-4 rounded-full bg-gradient-to-r from-[#2563EB] to-[#8B5CF6] px-3 py-1 text-xs font-black text-white">Popular</span> : null}
      <h3 className="pr-20 text-xl font-black text-white">{pack.title[locale]}</h3>
      <p className="mt-3 text-2xl font-black text-violet-200">{pack.price[locale]}</p>
...
export function BlogCard({ post, locale }: { post: BlogPost; locale: Locale }) {
  return (
    <Link href={localizedPath(locale, `blog/${post.slug}`)} className="group glass block h-full rounded-2xl p-6 transition duration-200 hover:-translate-y-1">
      <p className="text-xs font-bold uppercase tracking-[0.18em] text-violet-200">{new Date(post.date).getFullYear()}</p>
